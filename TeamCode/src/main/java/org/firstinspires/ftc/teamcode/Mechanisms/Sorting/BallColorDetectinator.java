...
public class BallColorDetectinator {
    ...
    public void addSensor(Supplier<Color> sensor) {
        sensors.add(sensor);
    }
    public void setup() {
        sensors = new HashSet<>();
    ...
    public void update() {
        for (Supplier<Color> sensor : sensors) {
            ...
            if (colormap.containsKey(sensor)) {
    ...
    public Color pullData(Supplier<Color> sensor) {
        Color sum  = new Color(0,0,0, ColorType.HSV);
